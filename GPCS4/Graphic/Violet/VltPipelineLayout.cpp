...
LOG_CHANNEL(Graphic.Violet.VltPipelineLayout);

namespace vlt
{;


VltDescriptorSlotMap::VltDescriptorSlotMap()
{
...
uint32_t VltDescriptorSlotMap::bindingCount() const
{
	return m_slots.size();
}

const VltDescriptorSlot* VltDescriptorSlotMap::bindingInfos() const
{
	return m_slots.data();
}

void VltDescriptorSlotMap::defineSlot(const VltResourceSlot& slot, VkShaderStageFlags stage)
{
	m_slots.push_back({ slot, stage });
}

uint32_t VltDescriptorSlotMap::getBindingId(VkShaderStageFlagBits stage, uint32_t startRegister) const
{
