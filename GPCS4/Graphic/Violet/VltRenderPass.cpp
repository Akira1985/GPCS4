...
LOG_CHANNEL(Graphic.Violet.VltRenderPass);

namespace vlt
{;

bool VltRenderPassFormat::operator==(const VltRenderPassFormat& fmt) const
{
...
size_t VltRenderPassFormat::hash() const
{
	VltHashState state;
...
VltRenderPass::VltRenderPass(const RcPtr<VltDevice>& device, const VltRenderPassFormat& fmt):
	m_device(device),
	m_format(fmt),
	m_default(createRenderPass(VltRenderPassOps()))
{
}

VltRenderPass::~VltRenderPass()
{
...
bool VltRenderPass::hasCompatibleFormat(const VltRenderPassFormat& fmt) const
{
	return m_format == fmt;
}

VkRenderPass VltRenderPass::getHandle(const VltRenderPassOps& ops)
{
...
VkRenderPass VltRenderPass::createRenderPass(const VltRenderPassOps& ops)
{
...
bool VltRenderPass::compareOps(const VltRenderPassOps& a, const VltRenderPassOps& b)
{
...
VltRenderPassPool::VltRenderPassPool(VltDevice* device):
	m_device(device)
{

}

VltRenderPassPool::~VltRenderPassPool()
{

}

VltRenderPass* VltRenderPassPool::getRenderPass(const VltRenderPassFormat& fmt)
{
	VltRenderPass* renderPass = nullptr;
	do 
	{
