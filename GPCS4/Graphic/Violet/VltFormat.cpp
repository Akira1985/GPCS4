...
const std::array<VltFormatInfo, 149> g_formatInfos = 
{ {
	// VK_FORMAT_UNDEFINED
	{},
	...
	// VK_FORMAT_R8_UINT
	{ 1, VK_IMAGE_ASPECT_COLOR_BIT,
	  VltFormatFlag::SampledUInt },

	// VK_FORMAT_R8_SINT
	{ 1, VK_IMAGE_ASPECT_COLOR_BIT,
	  VltFormatFlag::SampledSInt },

	// VK_FORMAT_R8_SRGB
	{ 1, VK_IMAGE_ASPECT_COLOR_BIT,
	  VltFormatFlag::ColorSpaceSrgb },
...
{
	const VltFormatInfo* formatInfo  = nullptr;
	...
	for (const auto& group : g_formatGroups)
	{
		if (format >= group.first && format <= group.second)
		{
			...
			formatInfo = &g_formatInfos[indexOffset + index];
			break;
		}
		else
		{
			indexOffset += uint32_t(group.second) - uint32_t(group.first) + 1;
		}
	}
